Implement a single GPT-2 transformer decoder block. Given an input tensor $x$ of shape `(seq_len, 768)` and a packed weight buffer containing all block parameters, compute the output using pre-norm architecture with multi-head self-attention and a feed-forward network with GELU activation.

实现一个 GPT-2 Transformer 解码器模块。给定形状为 `(seq_len, 768)` 的输入张量 $x$ 和包含模块全部参数的打包权重缓冲区，使用带多头自注意力和 GELU 激活前馈网络的 pre-norm 架构计算输出。

![](data:image/svg+xml;base64,)

The block uses GPT-2's **pre-norm** architecture: LayerNorm is applied *before* each sub-layer (attention and feed-forward), not after. At a high level:

该模块采用 GPT-2 的 **pre-norm** 架构：LayerNorm 应用于每个子层（注意力层和前馈层）*之前*，而不是之后。整体计算如下：

$$
\begin{aligned}
x' &= x + \text{MultiHeadAttn}\!\left(\text{LN}_1(x)\right) \\[4pt]
\text{output} &= x' + \text{FeedForward}\!\left(\text{LN}_2(x')\right)
\end{aligned}
$$

where the sub-layers are defined as:

其中各子层定义如下：

$$
\begin{aligned}
\text{LN}(z) &= \frac{z - \mu}{\sqrt{\sigma^2 + \epsilon}} \odot \gamma + \beta, \quad \mu = \frac{1}{d}\sum_i z_i, \quad \sigma^2 = \frac{1}{d}\sum_i (z_i - \mu)^2 \\[8pt]
[Q \mid K \mid V] &= \text{LN}_1(x) \cdot W_{qkv} + b_{qkv} \\[4pt]
\text{head}_i &= \text{softmax}\!\left(\frac{Q_i K_i^\top}{\sqrt{d_k}}\right) V_i, \quad d_k = 64 \\[4pt]
\text{MultiHeadAttn}(z) &= \text{Concat}(\text{head}_1, \ldots, \text{head}_{12}) \cdot W_{\text{attn}} + b_{\text{attn}} \\[8pt]
\text{FeedForward}(z) &= \text{GELU}\!\left(z \cdot W_{fc} + b_{fc}\right) \cdot W_{\text{proj}} + b_{\text{proj}}
\end{aligned}
$$

Expanding into individual steps:

展开为以下具体步骤：

1.  **Layer Norm 1 / 层归一化 1：** $x_{\text{norm}} = \text{LN}_1(x)$ with parameters $\gamma_1, \beta_1$ / 使用参数 $\gamma_1, \beta_1$ 计算 $x_{\text{norm}} = \text{LN}_1(x)$。
2.  **QKV Projection / QKV 投影：** $QKV = x_{\text{norm}} \cdot W_{qkv} + b_{qkv}$, split into $Q, K, V$ each of shape `(seq_len, 768)` / 计算 $QKV = x_{\text{norm}} \cdot W_{qkv} + b_{qkv}$，并将其拆分为形状均为 `(seq_len, 768)` 的 $Q$、$K$、$V$。
3.  **Multi-Head Attention / 多头注意力：** Reshape $Q, K, V$ into 12 heads of dimension 64, compute per-head scaled dot-product attention (no causal mask), then concatenate heads into $A$ / 将 $Q$、$K$、$V$ 重塑为 12 个、每个维度为 64 的注意力头，计算每个头的缩放点积注意力（不使用因果掩码），再将各头拼接为 $A$。
4.  **Output Projection / 输出投影：** $P = A \cdot W_{\text{attn}} + b_{\text{attn}}$
5.  **Residual 1 / 残差连接 1：** $x' = x + P$
6.  **Layer Norm 2 / 层归一化 2：** $h_{\text{norm}} = \text{LN}_2(x')$ with parameters $\gamma_2, \beta_2$ / 使用参数 $\gamma_2, \beta_2$ 计算 $h_{\text{norm}} = \text{LN}_2(x')$。
7.  **Feed-Forward / 前馈网络：** $F = \text{GELU}(h_{\text{norm}} \cdot W_{fc} + b_{fc}) \cdot W_{\text{proj}} + b_{\text{proj}}$
8.  **Residual 2 / 残差连接 2：** $\text{output} = x' + F$

## Implementation Requirements / 实现要求

- Use only native features (external libraries are not permitted) / 只能使用原生功能（不得使用外部库）
- The `solve` function signature must remain unchanged / `solve` 函数签名必须保持不变
- The final result must be stored in the `output` tensor / 最终结果必须存储在 `output` 张量中
- LayerNorm uses $\epsilon = 10^{-5}$ / LayerNorm 使用 $\epsilon = 10^{-5}$
- Use the <a href="https://docs.pytorch.org/docs/stable/generated/torch.nn.GELU.html" target="_blank">GELU tanh approximation</a>: $\text{GELU}(x) = 0.5\,x\!\left(1 + \tanh\!\left(\sqrt{\tfrac{2}{\pi}}\left(x + 0.044715\,x^3\right)\right)\right)$ / 使用 <a href="https://docs.pytorch.org/docs/stable/generated/torch.nn.GELU.html" target="_blank">GELU 的 tanh 近似</a>：$\text{GELU}(x) = 0.5\,x\!\left(1 + \tanh\!\left(\sqrt{\tfrac{2}{\pi}}\left(x + 0.044715\,x^3\right)\right)\right)$

## Weight Layout / 权重布局

All block parameters are packed into a single contiguous `weights` buffer (7,087,872 floats) in the following order. Index into the buffer using the offsets below (e.g. $W_{qkv}[i][j]$ is at `weights[1536 + i * 2304 + j]`). All 2D matrices are stored in row-major order.

模块的所有参数按以下顺序打包到一个连续的 `weights` 缓冲区中（共 7,087,872 个浮点数）。使用下表中的偏移量索引该缓冲区（例如，$W_{qkv}[i][j]$ 位于 `weights[1536 + i * 2304 + j]`）。所有二维矩阵均按行优先顺序存储。

| Parameter / 参数                            | Shape / 形状       |      Size / 大小 |    Offset / 偏移 |
|--------------------------------------|-------------|----------:|----------:|
| $\gamma_1$ (LN1 weight / 权重) | (768,)      |       768 |         0 |
| $\beta_1$ (LN1 bias / 偏置)   | (768,)      |       768 |       768 |
| $W_{qkv}$              | (768, 2304) | 1,769,472 |     1,536 |
| $b_{qkv}$ (bias / 偏置)              | (2304,)     |     2,304 | 1,771,008 |
| $W_{\text{attn}}$              | (768, 768)  |   589,824 | 1,773,312 |
| $b_{\text{attn}}$ (bias / 偏置)              | (768,)      |       768 | 2,363,136 |
| $\gamma_2$ (LN2 weight / 权重) | (768,)      |       768 | 2,363,904 |
| $\beta_2$ (LN2 bias / 偏置)   | (768,)      |       768 | 2,364,672 |
| $W_{fc}$              | (768, 3072) | 2,359,296 | 2,365,440 |
| $b_{fc}$ (bias / 偏置)              | (3072,)     |     3,072 | 4,724,736 |
| $W_{\text{proj}}$              | (3072, 768) | 2,359,296 | 4,727,808 |
| $b_{\text{proj}}$ (bias / 偏置)              | (768,)      |       768 | 7,087,104 |

## Example / 示例

With `seq_len` = 4, `x` uniformly drawn from \[−1, 1\], and weights randomly initialized (see Weight Layout for the packing structure):

当 `seq_len` = 4 时，$x$ 从 \[−1, 1\] 均匀采样，权重随机初始化（打包结构见“权重布局”）：

    Input:  x.shape       = (4, 768)       # 4 token embeddings
            weights.shape = (7,087,872,)   # packed weight buffer
            seq_len       = 4
    Output: output.shape  = (4, 768)       # transformed token embeddings

    输出：output.shape  = (4, 768)       # 变换后的 token 嵌入

## Constraints / 约束

- `d_model` = 768, `n_heads` = 12, `ffn_dim` = 3,072 (GPT-2 124M architecture) / `d_model` = 768、`n_heads` = 12、`ffn_dim` = 3,072（GPT-2 1.24 亿参数架构）
- 1 ≤ `seq_len` ≤ 4,096 / `seq_len` 的范围为 1 至 4,096
- All tensors use 32-bit floating point / 所有张量均使用 32 位浮点数
- Performance is measured with `seq_len` = 1,024 / 性能测试使用 `seq_len` = 1,024
